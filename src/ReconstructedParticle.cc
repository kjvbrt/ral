...
namespace ROOTDataFrameAnalysis{
  ...
  namespace ReconstructedParticle{
	...
	for(edm4hep::ReconstructedParticleData p : particles)
	{
	  result.emplace_back(ROOT::Math::XYZVector(p.momentum.x, p.momentum.y, p.momentum.z));
	}
	return result;
    }
	...
	for(edm4hep::ReconstructedParticleData p : particles)
	{
	  result.emplace_back(ROOT::Math::XYZVector(p.referencePoint.x, p.referencePoint.y, p.referencePoint.z));
	}
	return result;
    }
